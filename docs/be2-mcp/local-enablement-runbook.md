...
```bash
# 開發模式(tsx 直跑;host 由 .env 的 AUTHSVC_URL/GATEWAY_URL 給,已指 be2-220)
APP_DB_PATH=./data/be2-mcp-sit-220.sqlite npm run dev
# → listening on http://127.0.0.1:8787/mcp (bind 127.0.0.1:8787, env: api-gateway-220.sit)

# 或:模擬 production 產物(容器就是這樣跑)
npm run build && APP_DB_PATH=./data/be2-mcp-sit-220.sqlite node dist/src/index.js
```
...
```bash
APP_DB_PATH=./data/be2-mcp-sit-220.sqlite APP_PORT=8788 npm run dev
...
| env | 用途 | 本機值 |
|---|---|---|
| `APP_ENV` | 環境標籤(只影響預設 db 路徑後綴 `sit\|stage\|prod`,不再選 host/key) | 不設;既有 sit-220 db 用 `APP_DB_PATH` 明指 |
| `AUTHSVC_URL` / `GATEWAY_URL` | 環境 host(一環境一份,`.env` 直接給) | `auth-220.sit` / `api-gateway-220.sit` |
| `APP_DB_PATH` | SQLite 路徑 | `./data/be2-mcp-sit-220.sqlite`(沿用既有本機 db) |
