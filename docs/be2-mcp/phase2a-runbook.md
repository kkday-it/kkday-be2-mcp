...
- **`modify_user` is a documented placeholder that now fails loud, not silent.** The executor takes an injected `modifyUserFrom(accessToken)` resolver. The real resolution of a be2 `userUuid` from the auth-service session (as opposed to a JWT claim — none of the JWT claims match the stored `modify_user` UUID, see `docs/be2-mcp/sit-write-contracts.md` #1) is unresolved pending a write-capable SIT account; wiring the real resolver into `app.ts` is deferred. Because a wrong-but-syntactically-valid `modify_user` on a live write would be a silent wrong-attribution bug, the placeholder in `app.ts` (`modifyUserFromPlaceholder`) now **throws `AppError('MODIFY_USER_UNRESOLVED')`** unless the environment variable `BE2_MCP_ALLOW_PLACEHOLDER_MODIFY_USER=1` is set. The executor's existing stuck-state guard (the same one covering token-refresh failure) catches this throw, marks the change-set `failed` (not stuck in `executing`), audits it, and rethrows — see `tests/changesetExecutor.test.ts` and `tests/modifyUserPlaceholder.test.ts`. Do **not** set this flag against a write-capable account until the real resolver is wired in.
