...
|---|---|---|
| `be2_create_changeset` | `action_type: "shelf_toggle_product" \| "shelf_toggle_plan"`, `items: [{prod_oid, pkg_oid?, target_is_active}]` (1–20), `note?` | Computes a live diff (current vs target) for each item, stages it as a `pending_approval` change-set, and returns a `confirm_url` + `diff_version` + the diff itself. Does **not** touch be2. |
...
2. **Ask for the change.** e.g. "把商品 `<prodOid>` 下架" or "把方案 `<pkgOid>` 上架". The agent calls `be2_create_changeset` and returns a `confirm_url`. **The agent will not, and cannot, claim the change is done at this point** — nothing has executed yet.
3. **Open `confirm_url` in a browser** (`http://127.0.0.1:<port>/confirm/<id>?token=<token>` — a one-time high-entropy capability link, not tied to a login session). The page recomputes the diff live against be2 at render time (not just what was cached at creation) and shows current → target per item, by oid (names are untrusted be2 content, shown for orientation only — verify by oid).
...
| Confirmation page: `404 not found` | Wrong/garbage token, unknown `id`, or the change-set is no longer `pending_approval` (already approved/rejected/expired) | Re-check the `confirm_url` was copied in full; if it was already decided, create a new change-set |
...
- **`modify_user` is a documented placeholder.** The executor takes an injected `modifyUserFrom(accessToken)` resolver. The real resolution of a be2 `userUuid` from the auth-service session (as opposed to a JWT claim — none of the JWT claims match the stored `modify_user` UUID, see `docs/be2-mcp/sit-write-contracts.md` #1) is unresolved pending a write-capable SIT account; wiring the real resolver into `app.ts` is deferred.
...
3. `把方案 <pkgOid> 下架` → confirm the agent calls `be2_create_changeset` (`action_type=shelf_toggle_plan`), returns a `confirm_url`, and does **not** claim the change already happened.
4. Open the `confirm_url` in a browser → verify the diff page shows the plan name + current→target, and inspect response headers for `Referrer-Policy: no-referrer`.
...
7. **Revert**: create a new change-set with the inverse `target_is_active` for the same `pkg_oid`, open its `confirm_url`, approve it, and confirm the plan is restored to its original state (compare against the "before" captured in step 2/6).
