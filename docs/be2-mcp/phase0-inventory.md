...
**Phase 4a 進度（2026-08-14/15,Task 1–8）**：**已實作完成 + Live SIT be2-220 e2e 驗收（`shelf_schedule` 通過，`inventory_platform` 如預期 fail-closed）**——BAA（BE2 Action Assistant）批次精靈搬進 MCP Apps 互動面板（24h POC，spec/plan 皆已 agy approved），兩個新 change-set `action_type`：`inventory_platform`（庫存管理平台三態切換 BE2/BE2_SCM/EXTERNAL，變更單位 `item_oid×supplier_oid`、面板自動連動共用 item 的兄弟方案）、`shelf_schedule`（be2 原生排程上下架，`reserve_queue` 整組取代、依 `prod_oid` 分組單 PUT 帶多 pkg 原生批次）；2 個 app-only tools（`app_get_batch_view` 三層 read-oids 登記、`app_create_changeset` 復用 `createChangesetCore`）+ 1 個 model-visible 入口（`be2_open_batch_wizard`）；四步驟批次精靈面板（`ui://be2/batch-wizard.html`：選擇→檢視→批准→結果，含篩選/隱藏未勾選、item×supplier 連動勾選對稱雙向、shelf_schedule 時區雙顯示）。Task 1 probe 定案 `inventory_platform` 現況讀取 = `GET items/{itemOid}/configs → supplier_configs[]`（`packages?show_supplier=1` 只用於面板初始清單，非 diff/批准的 live 重算來源）。`npm run ci` **392 passed / 0 skipped**、`tsc` clean；`npm run eval` 因無 `ANTHROPIC_API_KEY` 為文件化 SKIP（非失敗），新增 2 個批次精靈 eval case（拒絕未經批准即宣稱完成、拒絕直接寫 reserve-active 引導走精靈）。Pilot 文件見 `docs/be2-mcp/phase4a-runbook.md`。
**Live 驗收結果（2026-08-14/15，`scripts/live-4a-acceptance.ts`，對 34133）**：`shelf_schedule` **全鏈路 live 驗收通過**（`RESULT=SHELF_SCHEDULE_LIVE_OK`）——建 change-set → 批准 → 執行（真 200）→ 讀回驗證排程已寫入 → 建第二個 change-set 還原 → 批准 → 執行 → 讀回驗證已還原成原始狀態，可逆性成立、無 mock。`inventory_platform` **建立階段被 `DiffError` fail-closed 擋下**（`GET items/{itemOid}/configs` 對 `.env` 帳號在 be2-220 回 403，verify per-URI 授權缺，與 Phase 3a 庫存數量寫入的 `AU9403` 同一類卡點）——寫入契約本身已 live 實證可用（design spec §2.1，200），純粹是這個帳號在這個環境對這個讀取端點缺授權；這是 spec §4.1「讀取失敗→擋下建立、嚴禁假設預設值」被正確執行的展示，非程式缺陷。解卡路徑同 Phase 3a：be2-220 補該 action、或改打已知授權完整的 stage。細節與 sanitized 輸出見 `docs/be2-mcp/phase4a-runbook.md`「Live 驗收結果」節、`docs/be2-mcp/sit-write-contracts.md`「inventory-platform read」節。
