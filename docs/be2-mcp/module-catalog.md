...
| `announcement_update` | `announcementOid`(PK,必填)、同 `announcement` 其餘欄位（全量必填，非 partial） | `announce_update:announcementOid:name:prod_oids(sorted):start_time` | `product.announcement.update`<br>onMissing: **`warn`**（同 create） | **綁 live current**（跟 create 的 target-only 不同）：GET detail 讀現況、hash 同時納入 target payload 與 current 快照——live 現況被別人改過也會使 diffVersion 改變（stale 偵測） | 逐筆 read-merge-write：`getDetail(oid)`→現況、目標欄位覆蓋 → 整份文件 `patch(oid, body)`（**§6.2 full-REPLACE**：`langSettings` 整包送出、非部分合併；§6.3 讀寫不對稱：READ `langs`→WRITE `langSettings`、`prodOids` GET 回字串需 parse 成 int[]、`endTime` 無值送 `null` 非 `""`）；沿用 create 同一個 svc-b2c client（`getDetail`/`patch`）、403→per-item failed | **紅字高風險**：整份文件覆蓋、未列出的語系會被取代；確認頁顯示 before(現況)→after(更新後) | 無（沿用 create 的獨立公告入口即可涵蓋更新場景；本 module 本身未產專用 UI） |
