...
> 環境:本機 :8787（**current main 碼**、`APP_ENV=sit-220`、dev panel 開、fresh token）。對 be2-220。
...
- 呼叫回 `REAUTH_REQUIRED`（token 過期,約 50 分）→ 終端跑 `APP_ENV=sit-220 npm run bootstrap-user` 重取,再重載面板。
