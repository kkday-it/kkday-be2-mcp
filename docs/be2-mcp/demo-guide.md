...
- [ ] **標的商品準備**：預選可安全操作的測試商品 OID `34133` (以及備用的 `9468`)。
...
- **畫面**：輸入「幫我把 34133 這些方案排程明天中午 12 點上架」。
- **操作**：
  - Desktop 彈出互動精靈面板。
  - 在面板上檢視差異 (Diff)，並點擊批准。
  - 成功後自動觸發「讀回驗證 (✓)」。
...
| 突發狀況 | 備案處理 | 備註 |
|---|---|---|
| **SIT 掛了或 Gateway timeout** | 改開 `BE2_MCP_DEV_PANEL=1` 演示 `http://127.0.0.1:8787/dev/panel/batch-wizard` harness。 | 證明面板渲染與機制正常，僅後端無回應。 |
