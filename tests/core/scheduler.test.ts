...
describe('scheduler', () => {
  ...
  function seed(id: string, executeAtUtc: number, accessExpiresAt: number = 9999999) {
    identities.upsert({ identityId: 'id-1', userLabel: 'u', accessToken: 'acc', refreshToken: 'ref', businessList: [], accessExpiresAt, updatedAt: currentTime })
    changeSets.create({
    ...
    changeSets.setScheduled(id, { identityId: 'id-1', userLabel: 'u', modifyUser: 'mu', sessionId: 'sess' }, currentTime)
  }
  ...
  it('1. 到點執行:建 scheduled(executeAtUtc=T)、identity access 未過期 → tick(now=T) 後 status ∈ done 且 gateway PUT 被呼叫', async () => {
    ...
    currentTime = 2000
    await s.tick()
    expect(changeSets.get('c1')?.status).toBe('done')
  ...
  it('2. 未到點:tick(now=T-1) → 仍 scheduled、PUT 未被呼叫', async () => {
    ...
    currentTime = 1999
    await s.tick()
    expect(changeSets.get('c2')?.status).toBe('scheduled')
  ...
  it('3. grace 超窗:tick(now=T+graceMs+1) → status=missed、PUT 未被呼叫、audit 有 schedule.missed', async () => {
    ...
    await s.tick()
    expect(changeSets.get('c3')?.status).toBe('missed')
    expect(putCount).toBe(0)
    const row = db.prepare('SELECT * FROM audit_log WHERE tool=?').get('schedule.missed')
  ...
  it('4. terminal refresh 失敗:identity access 已過期 + fake auth refresh 丟 AuthError 401 → status=failed, audit 記 AUTH_EXPIRED; PUT 未被呼叫', async () => {
    seed('c4', 2000, 1000) // already expired
    fakeAuth.refresh.mockRejectedValue(new AuthError('REVOKED', 'revoked', 401))
    const s = makeScheduler(deps, p)
    currentTime = 2000
    await s.tick()
    expect(changeSets.get('c4')?.status).toBe('failed')
    expect(putCount).toBe(0)
    const row = db.prepare('SELECT * FROM audit_log WHERE tool=? AND status=?').get('schedule.execute', 'error')
  ...
  it('6. stranded 回收:手動 claimScheduled 後不執行 → tick(now=claim+staleClaimMs+1) 放回並重新認領執行', async () => {
    seed('c6', 2000)
    currentTime = 2000
    changeSets.claimScheduled('c6', currentTime)
    expect(changeSets.get('c6')?.status).toBe('approved') // executing state internally
    
    const s = makeScheduler(deps, p)
    currentTime = 2000 + p.staleClaimMs + 1
    await s.tick()
    expect(changeSets.get('c6')?.status).toBe('done')
    expect(putCount).toBe(1)
    const row = db.prepare('SELECT * FROM audit_log WHERE tool=?').get('schedule.reclaim')
    expect(row).toBeDefined()
  })
  ...
  it('7. 併發認領:兩個 makeScheduler 共用同一 db,同 tick 併發 → PUT 僅一次(CAS 去重)', async () => {
    ...
    expect(changeSets.get('c7')?.status).toBe('done')
    expect(putCount).toBe(1)
  })

  it('8. keep-alive:scheduled 未到點 + identity 將於 window 內到期 → tick 觸發 refresh 且 audit 記 schedule.keepalive; 第二 tick 不重複', async () => {
    seed('c8', 9_000_000, 1000 + 30_000) // windowMs is 60_000, so expires within window
    const s = makeScheduler(deps, p)
    currentTime = 1000
    await s.tick()
    expect(fakeAuth.refresh).toHaveBeenCalledTimes(1)
    expect(changeSets.get('c8')?.status).toBe('scheduled') // not executed yet
    
    const row = db.prepare('SELECT * FROM audit_log WHERE tool=?').get('schedule.keepalive')
