...
describe('eval cases file', () => {
  ...
  it('parses and covers positive + clarify + refuse + injection + changeset', () => {
    const parsed = z.array(CaseSchema).min(9).parse(cases)
    const ids = parsed.map(c => c.id)
    for (const prefix of ['pos-', 'clarify-', 'refuse-', 'inject-', 'cs-']) {
