...
async function mkCtx(over: Partial<L2ToolContext> = {}) {
  ...
  await readOids.record('s1', ['i1', 'p', 'k'])   // shelf_toggle 的 scopeOids 含 pkg_oid,'k' 也要登記
...
describe('createChangesetSchedule', () => {
  it('schedule on a schedulable module: converts wall→UTC and persists ScheduleInfo', async () => {
    ...
    const rec = (await ctx.changeSets.get((env.items[0] as { changeset_id: string }).changeset_id))!
  ...
  it('works via app_create_changeset tool (AppToolContext)', async () => {
    ...
    const rec = (await appCtx.changeSets.get((env.items[0] as { changeset_id: string }).changeset_id))!
    expect(rec.schedule).toEqual({ executeAtUtc: T0 + 3600_000, wall: '2026-09-01T09:00', tz: 'Asia/Taipei' })
  })
})
