...
async function seed(id: string, status: 'pending_approval' | 'scheduled' | 'cancelled', creatorLabel = 'owner@kkday.com') {
  await store.create({
  ...
  if (status === 'scheduled') {
    await store.setScheduled(id, { identityId: 'id-1', userLabel: creatorLabel, modifyUser: 'mu', sessionId: 's' }, T0)
  }
  if (status === 'cancelled') {
    await store.setScheduled(id, { identityId: 'id-1', userLabel: creatorLabel, modifyUser: 'mu', sessionId: 's' }, T0)
    await store.casStatus(id, 'scheduled', 'cancelled', T0)
  }
}
...
describe('scheduleCancel (confirm routes)', () => {
  ...
  it('2. POST /confirm/:id/cancel(scheduled)→ 200,status=cancelled,audit 記 changeset.cancel; 已 cancelled 再 POST → 409; pending_approval → 409', async () => {
    await seed('cs-2', 'scheduled')
    const res = await fetch(`${base}/confirm/cs-2/cancel`, { method: 'POST', headers: { cookie: 'be2mcp_sid=sid-owner' } })
    expect(res.status).toBe(200)
    expect((await store.get('cs-2'))!.status).toBe('cancelled')

    // Check audit
    const log = (await db.query('SELECT * FROM audit_log WHERE tool = $1 AND status = $2', ['changeset.cancel', 'ok'])).rows[0]
    ...
    await seed('cs-3', 'pending_approval')
...
describe('scheduleCancel (app tools)', () => {
  ...
  it('3. 面板:app_get_changeset_view 對 scheduled 回 schedule + diff_version + nonce; app_confirm_changeset decision=cancel → cancelled; 對 pending_approval → NOT_CANCELLABLE', async () => {
    ...
    expect((await store.get('cs-4'))!.status).toBe('cancelled')

    await seed('cs-5', 'pending_approval')
  ...
  it('4. 取消後 scheduler tick 不執行', async () => {
    await seed('cs-6', 'scheduled')
    const { ctx } = mkCtx()

    // Check initially listed
    const due = await store.listDueScheduled(T0 + 3600_000 + 1)
    ...
    // Check again
    const dueAfter = await store.listDueScheduled(T0 + 3600_000 + 1)
    expect(dueAfter.length).toBe(0)
    expect((await store.get('cs-6'))!.status).toBe('cancelled')
  })
})
