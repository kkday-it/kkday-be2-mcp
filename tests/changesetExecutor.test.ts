...
async function deps(gwState: Record<string, any>, over: Partial<ExecutorDeps> = {}): Promise<{ deps: ExecutorDeps; store: ChangeSetStore }> {
...
async function seedProduct(store: ChangeSetStore, target: boolean) {
  await store.create({ id: 'cs1', creatorLabel: 'owner@kkday.com', creatorBearerHash: 'bh', sessionId: 's', actionType: 'shelf_toggle_product',
...
describe('executeChangeSet', () => {
  ...
  it('no-op is skipped (no write) when already in target', async () => {
    const { deps: d, store } = await deps({ '/product/api/v1/product-configs/p1/switch': { is_active: false } })
    await seedProduct(store, false)
    const out = (await executeChangeSet(d, 'cs1', WHO))!
  ...
  it('refuses to execute a non-approved change-set', async () => {
    const { deps: d, store } = await deps({ '/product/api/v1/product-configs/p1/switch': { is_active: true } })
    await seedProduct(store, false); await store.setStatus('cs1', 'pending_approval')
  ...
  it('I-2: a failed inventory item (quantity PUT rejected) audits as status "error" (not "ok")', async () => {
    ...
    const row = (await audit.recent()).find(r => r.tool === 'changeset.execute' && (r.params as { item?: string }).item === 'i1:s1')
